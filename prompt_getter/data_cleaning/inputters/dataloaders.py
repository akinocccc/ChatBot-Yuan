...
def paths_dataloader(dir_path, out_dir, batch_size):
    ...
    # jsonl_path_list = [(file, subdir, os.path.join(subdir_path, file))
    #                    for subdir, subdir_path in subdirs
    #                    for file in os.listdir(subdir_path) if file.endswith(".txt")]
                       # for file in os.listdir(subdir_path) if file.endswith(".jsonl")]
    jsonl_path_list = [(file, os.path.join(dir_path, file))
                       for file in os.listdir(dir_path) if file.endswith(".txt")]
    # random.shuffle(jsonl_path_list)
    ...
    for file, path in jsonl_path_list:
        ...
        for i in range(0, file_len, batch_size):
            ...
            fid = "_" + file.replace(".jsonl", "") + "_trunc" + m2.hexdigest()
            # out
            out_subdir = os.path.join(cleaned_dir)
            if not os.path.exists(out_subdir):
                os.mkdir(out_subdir)
    ...
    #         m2 = hashlib.md5()
    #         m2.update(str(time.time()).encode('utf-8'))
    #         fid = subdir_name + "_" + file.replace(".jsonl", "") + "_trunc" + m2.hexdigest()
    #         # out
    #         out_subdir = os.path.join(cleaned_dir, subdir_name)
    #         if not os.path.exists(out_subdir):
    #             os.mkdir(out_subdir)
